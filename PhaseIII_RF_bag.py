...
rf_grid_adjusted = RandomForestClassifier(random_state=5805)
grid_search_adjusted = GridSearchCV(rf_grid_adjusted, param_grid_adjusted, cv=5, scoring='accuracy', n_jobs=-1)
grid_search_adjusted.fit(X_train, y_train)


best_params_adjusted = grid_search_adjusted.best_params_
best_rf_adjusted = grid_search_adjusted.best_estimator_
...
for i, (train, test) in enumerate(cv.split(X, y_binary)):
    y_pred = best_rf_adjusted.predict(X.iloc[test])
    ...
    y_probas = best_rf_adjusted.predict_proba(X.iloc[test])[:, 1]
