...
# Adjusted Grid Search with cross-validation for Logistic Regression
grid_search_adjusted = GridSearchCV(stacking_clf, param_grid_adjusted, cv=5, scoring='accuracy', n_jobs=-1)
...
best_stacking_adjusted = grid_search_adjusted.best_estimator_
...
for i, (train, test) in enumerate(cv.split(X, y_binary)):
    y_pred = best_stacking_adjusted.predict(X.iloc[test])
    ...
    y_probas = best_stacking_adjusted.predict_proba(X.iloc[test])[:, 1]
    fpr, tpr, thresholds = roc_curve(y_binary.iloc[test], y_probas)
