...
param_grid_adjusted = {
    'hidden_layer_sizes': [(100,), (100, 100)],  # Example sizes of hidden layers
...
mlp_grid_adjusted = MLPClassifier(max_iter=1000, random_state=5805)
grid_search_adjusted = GridSearchCV(mlp_grid_adjusted, param_grid_adjusted, cv=5, scoring='accuracy', n_jobs=-1)
grid_search_adjusted.fit(X_train, y_train)


best_params_adjusted = grid_search_adjusted.best_params_
best_mlp_adjusted = grid_search_adjusted.best_estimator_
...
for i, (train, test) in enumerate(cv.split(X, y_binary)):
    y_pred = best_mlp_adjusted.predict(X.iloc[test])
    ...
    fpr, tpr, thresholds = roc_curve(y_binary[test], best_mlp_adjusted.predict_proba(X.iloc[test])[:, 1], pos_label=1)
